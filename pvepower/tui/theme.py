...
# 设计系统的原始十六进制值。终端用不上（见下面的 LIGHT），但 HTML 邮件
# 用得上——日报正文就是按这套色渲染的，这样邮件和界面是同一套视觉。
# 放在这里而不是各写一份，是为了改配色时只有一个地方要改。
# 和终端那边一样分两档：文字档对 bg 做到 4.5:1，图形档（色块、进度条）
# 按 3:1 收。设计系统给的 accent #D97706 对 #F8FAFC 只有 3.04:1，当正文
# 色不够，所以文字用深一档的 accent_text。tests/test_theme.py 会核对。
DESIGN_HEX = {
    "primary":     "#1E40AF",   # 8.3:1  标题、强调数字
    "primary_fg":  "#FFFFFF",   # 8.7:1  主色块上的白字
    "secondary":   "#3B82F6",   # 3.5:1  图形档：柱子填充
    "accent":      "#D97706",   # 3.0:1  图形档：强调色块
    "accent_text": "#B45309",   # 4.8:1  文字档的琥珀
    "bg":          "#F8FAFC",   # 页面底
    "fg":          "#1E3A8A",   # 9.9:1  深藏青标题
    "text":        "#334155",   # 9.9:1  正文
    "muted":       "#E9EEF6",   # 表头 / 分组带底色
    "border":      "#DBEAFE",   # 分隔线
    "ok":          "#15803D",   # 4.8:1  正常
    "destructive": "#DC2626",   # 4.6:1  故障
}
