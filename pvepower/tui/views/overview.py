"""Overview: live power, today's cost, and machine health at a glance."""
...
class OverviewView(View):
    ...
    def draw(self, win, height: int, width: int) -> None:
        ...
        box_h = 8
        draw_box(win, 0, 0, box_h, width, "Live Power",
                 color(CP_TITLE), color(CP_TITLE, bold=True))

        if watts is None:
            safe_addstr(win, 2, 2, "BMC power reading unavailable",
            ...
            detail = (
                f"min {reading.minimum}W   max {reading.maximum}W   "
                f"avg {reading.average}W   state {reading.state}"
            )
            safe_addstr(win, 2, 2 + len(big) + 3, truncate(detail, width - len(big) - 8),
                        color(CP_DIM))
            ...
            safe_addstr(win, 3, 2, "load ", color(CP_DIM))
            ...
            if len(history) > 1:
                ...
                safe_addstr(win, 5, 2, "trend", color(CP_DIM))
                safe_addstr(win, 5, 8, sparkline(values, spark_width),
                            color(CP_ACCENT))
                safe_addstr(
                    win, 6, 8,
                    f"{min(values):.0f}W .. {max(values):.0f}W over last "
    ...
    def _draw_consumption(self, win, y, x, h, w, cur) -> None:
        draw_box(win, y, x, h, w, "Consumption",
                 color(CP_TITLE), color(CP_TITLE, bold=True))
        today = self.data.today
        month = self.data.month
        rows = [
            ("Today", today.kwh, today.cost, today.avg_watts),
            ("This month", month.kwh, month.cost, month.avg_watts),
        ]
        line = y + 2
        for label, kwh, cost, avg in rows:
            ...
            line += 1
            if avg:
                safe_addstr(win, line, x + 4, f"avg {avg:.0f}W", color(CP_DIM))
                line += 1
        ...
        if projected is not None:
            safe_addstr(win, line, x + 2, "Projected", color(CP_NORMAL, bold=True))
            ...
            safe_addstr(win, line, x + 28, rpad(f"{projected:.2f} {cur}", 14),
                        color(CP_WARN))
            line += 1
            safe_addstr(win, line, x + 4, "(month-end, at current rate)",
                        color(CP_DIM))

    def _draw_status(self, win, y, x, h, w, ) -> None:
        draw_box(win, y, x, h, w, "Machine",
                 color(CP_TITLE), color(CP_TITLE, bold=True))
        ...
        items = [
            ...
            ("Power", chassis.get("System Power", "?")),
            ("Restore policy", chassis.get("Power Restore Policy", "?")),
            ("Last event", chassis.get("Last Power Event", "-") or "-"),
        ]
        for label, value in items:
            if line >= y + h - 1:
                break
            safe_addstr(win, line, x + 2, pad(label, 15), color(CP_DIM))
            attr = status_attr(value) if label == "Power" else color(CP_NORMAL)
        ...
        faults = [
            ...
            ("Cooling fault", chassis.get("Cooling/Fan Fault")),
        ...
        if line < y + h - 1:
            if active:
                safe_addstr(win, line, x + 2,
                            truncate("FAULT: " + ", ".join(active), w - 4),
                            color(CP_CRIT, bold=True))
            else:
                safe_addstr(win, line, x + 2, "No chassis faults reported",
                            color(CP_OK))

    def _draw_health(self, win, y, x, h, w) -> None:
        draw_box(win, y, x, h, w, "Collector",
                 color(CP_TITLE), color(CP_TITLE, bold=True))
        stats = self.data.storage_stats
        line = y + 2
        last = self.data.last_sample
        if last is None:
            safe_addstr(win, line, x + 2, "No samples recorded yet.",
                        color(CP_WARN, bold=True))
            safe_addstr(win, line + 1, x + 2,
                        "Start the collector: systemctl start pve-power-collector",
                        color(CP_DIM))
            return
        ...
        if age > interval * 3:
            attr, verdict = color(CP_CRIT, bold=True), "STALE"
        elif age > interval * 2:
            attr, verdict = color(CP_WARN), "lagging"
        else:
            attr, verdict = color(CP_OK), "healthy"

        safe_addstr(win, line, x + 2, pad("Last sample", 15), color(CP_DIM))
        safe_addstr(win, line, x + 18,
                    f"{humanize_ago(last.ts)}  ({verdict})", attr)
        line += 1
        safe_addstr(win, line, x + 2, pad("Coverage today", 15), color(CP_DIM))
        ...
        line += 1
        if line < y + h - 1:
            safe_addstr(win, line, x + 2, pad("Stored", 15), color(CP_DIM))
            safe_addstr(
                win, line, x + 18,
                f"{stats.get('samples', '0')} samples, "
                f"{stats.get('gaps', '0')} gaps, {stats.get('db_size', '-')}",
                color(CP_NORMAL),
            )
            line += 1
        if line < y + h - 1:
            ...
            safe_addstr(win, line, x + 2, pad("Tariff now", 15), color(CP_DIM))
            safe_addstr(win, line, x + 18,
